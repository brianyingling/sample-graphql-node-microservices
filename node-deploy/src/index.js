...
(async () => {
    ...
    if (fs.existsSync(lockFilePath)) {
        console.error('Lockfile deploy.lock has been found. Halting...');
        process.exit();
    }
    ...
    if (!fs.existsSync('.production.env')) {
        console.error('No .production.env file found. Halting...');
        process.exit();
    }
    ...
    await exec(`zip -r ${zipPath} . -x terraform/\\* -x node_modules/\\* -x \\*/node_modules/\\* -x \\*/.cache/\\* -x .git/\\* -x \\*.DS_Store`,
